...
void print_reports()
{
    ...
    FILE *StudentsOut= fopen("GradstudentsOut.txt", "w");
    printf_graduated_students(cout);
    //fprintf(output, "-----------------------------------------------------------------\n");
    ...
    fprintf(output, "-----------------------------------------------------------------\n");
    courses_taken_by_students(StudentsOut);
}
...
void courses_taken_by_students (FILE *out)
{
    ...
    fprintf(out, "The courses taken by each graduated student in each term:\n");
    for (x=0; x<student_num; x++)
    {
        ...
        if(is_graduated(h_std)){
        	int grad_list[student_num];
        	bool check=false; //check if h_std is already in the grad_list 
        	for (int j=0; j<student_num; j++){
        		check=false;
        		if((h_std==grad_list[j])&&(h_std>0)){
        			check=true;
        			continue;
     		 	}
     		}
     		if(!(check)){
				...
				grad_list[h_std]=h_std;
        fprintf(out, "Student B%d:\n", h_std+1);
        for (int j=0; j<students[h_std].taken_num; j++)
        {
...
void printf_graduated_students(ostream& out)
{
    int i, sum=0;
    for (i=0; i<student_num; i++)
    {
    ...
    out<< "Number of graduated students = "<<sum <<endl;
}
